...
class AttendanceRepository {
  ...
  AttendanceRepository({required Dio dio, required SecureStorageService secureStorage})
      : _dio = dio,
        _secureStorage = secureStorage;

  Future<AttendanceResponseModel> fetchChildAttendance({int page = 1, int limit = 20}) async {
    ...
    try {
      final Response<dynamic> response = await _dio.get<dynamic>(
        ApiEndpoints.attendanceMyChild,
      ...
      String message = 'Failed to load attendance data.';
      if (data is Map<String, dynamic>) {
